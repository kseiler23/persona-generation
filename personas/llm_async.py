"""Async wrapper for LLM calls using litellm."""
...
async def achat_completion(
    *,
    ...
    """
    Async wrapper around LiteLLM's completion API.

    This lets us make concurrent API calls for better performance.
    """
    ...
    if not content:
        ...
        completion = await litellm.acompletion(**retry_kwargs)
        choice = completion.choices[0]
        message = getattr(choice, "message", None)
        if isinstance(message, dict):
            content = message.get("content", "")
