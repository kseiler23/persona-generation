...
- **Simulated Doctor Agent**: An autonomous clinician agent that can interview the simulated patient to practice diagnosis and information gathering. Used for generating training data.
...
- **Doctor Mode**: Switch to "Doctor Training" to watch the Simulated Doctor interview the patient autonomously.
...
## Simulated Doctor & Training

The system now supports an automated **Simulated Doctor Agent** for generating training traces.

### Usage
1. **Auto-Play**: In the UI, set up a Patient Profile and BLP, then click "Simulate Doctor Session". The system will run a full dialogue until a diagnosis is reached.
2. **Reward/Critique**: The session ends with a report card (Diagnostic Accuracy, Process Quality) generated by an LLM Judge.
3. **Training**: Use the `/api/train/doctor` endpoint to run batch simulations. Traces are logged to `data/traces/` in JSONL format, suitable for GRPO/DPO fine-tuning.
